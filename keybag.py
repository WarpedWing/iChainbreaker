...
class _tlvint(BigEndianStructure):
    _fields_ = [("type", c_char * 4), ("length", c_uint32), ("value", c_uint32)]


class _tlvuuid(BigEndianStructure):
    _fields_ = [("type", c_char * 4), ("length", c_uint32), ("value", c_ubyte * 16)]


class _tlvsalt(BigEndianStructure):
    _fields_ = [("type", c_char * 4), ("length", c_uint32), ("value", c_ubyte * 20)]


class _tlvhmac(BigEndianStructure):
    _fields_ = [("type", c_char * 4), ("length", c_uint32), ("value", c_ubyte * 40)]
...
class _keybag_data(BigEndianStructure):
    _fields_ = [("data", c_char * 4), ("datasize", c_uint32)]
...
def tlvs(data):
    ...
    while data:
        try:
            t, length = struct.unpack('!4sI', data[:8])
            value = struct.unpack(f'!{length}s', data[8 : 8 + length])[0]
        except Exception:
            print("Unproper TLV structure found: ", (data,))
            break
        # Normalize type to text for comparisons under Py3
...
class Keybag:
    ...
    def load_keybag_header(self):
        ...
        keybag_header = _memcpy(
            self.fbuf[sizeof(_keybag_data) : sizeof(_keybag_data) + sizeof(_keybag_header)],
            _keybag_header,
        )
    ...
    def debug_print_header(self):
        print('[+] Keybag Header')
        print(f" [-] versions : {self.keybag['version']}")
        print(f" [-] type : {self.get_keybag_type(self.keybag['type'])}")
        print(f" [-] uuid : {uuid.UUID(bytes=bytes(bytearray(self.keybag['uuid'])))}")
        print(f" [-] hmac key : {self.keybag['hmck']}")
        # self.keybag['wrap'] is a descriptive string (from get_wrap_name)
        print(f" [-] wrap : {self.keybag['wrap']}")
    ...
    def fill_buffer(self, BufSize, PRFKey, PRFKeyLen, xorKey):
        ...
        while Count < BufSize:
            ...
            for inOffset in range(0, PRFKeyLen, 4):
                xoredPRFKey += self.xor(
                    PRFKey[inOffset : inOffset + 4], struct.pack('i', NewXorKey)
                )
            NewXorKey += 1
            NewHeap += xoredPRFKey
            Count += PRFKeyLen
    ...
    def tangle_with_hardware(self, PRFKey, PRFKeyLen, itercount):
        ...
        while itercount > 0:
            newxorkey, filled_buf = self.fill_buffer(
                DERIVATION_BUFFER_SIZE, PRFKey, PRFKeyLen, xorkey
            )
            xorkey = newxorkey
            ...
            if Count >= itercount:
                Count = itercount
            passcodePRFKey = self.xor_buffer(
                encrypted_buf, PRFKeyLen * nBlock, passcodePRFKey, Count
            )
...
def main():
    ...
    print(f'device key is {hexlify(devicekey)}')
